...
var Page = function() { var $page;
    this.a = function($this, t, p) {
        ...
        var $r = $page.find('.document-' + t).removeClass('document-' + t); 
        /*alert($r.outerHTML);
        $r.//each(function() { 
            //$(this).
            replaceWith(
               function() { return(this.outerHTML.replace('<div', '<' + t).replace('</div>', '</' + t + '>')); 
            }
            ); 
        //});*/
...
var Script = function(options) { var //Private
    ...
    out = function($Script) {  $('head').append($Script[0].outerHTML);
        $.log(' + : ' + $Script[0].outerHTML);
    },
